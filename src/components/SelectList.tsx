...
const SelectList: FC = () => {
  ...
  const selectChangeHandler = (e: FormEvent<HTMLSelectElement>) => {
    distpach(setSelectedList(e.currentTarget.value))
  }
      ...
      <div className="control has-icons-left">
        <div className="select fullwidth">
          <select className="fullwidth" onChange={selectChangeHandler}>
            ...
            {Object.keys(lists).length > 0 && Object.values(lists).map( (list: List) => <option key={list.id} value={list.id}>{list.name}</option>)}
          </select>
        </div>
